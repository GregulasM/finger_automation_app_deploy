...
export const useI18n = () => {
  ...
  const translations: Record<string, Record<string, string>> = {
    en: {
      ...
      // HTTP Request action
      "editor.http.urlHint": "Full URL including https://. Can include query params.",
      "editor.http.methodHint": "GET — fetch data, POST — send data, PUT — update, DELETE — remove",
      "editor.http.headersPlaceholder": '{"Authorization": "Bearer token", "X-Custom": "value"}',
      "editor.http.headersHint": "JSON object with HTTP headers. Content-Type is added automatically.",
      "editor.http.bodyPlaceholder": '{"key": "value"} or leave empty to forward previous step data',
      "editor.http.bodyHint": "Leave empty to automatically send data from the previous step.",
      ...
      // Email action
      "editor.email.toHint": "Recipient email address. You can use data from previous steps.",
      ...
      "editor.email.htmlPlaceholder": "<h1>Hello!</h1>\n<p>Your workflow completed successfully.</p>",
      "editor.email.htmlHint": "HTML content with formatting. Use tags like <b>, <p>, <a>.",
      "editor.email.textPlaceholder": "Hello!\n\nYour workflow completed successfully.",
      "editor.email.textHint": "Plain text fallback for email clients that don't support HTML.",
      "editor.email.autoContentHint": "💡 If content is empty, the data from the previous step will be sent as JSON automatically.",
      "editor.email.provider": "Email provider",
      "editor.email.gmailTitle": "Gmail SMTP",
      "editor.email.gmailDesc": "Send emails directly from your Gmail account using SMTP. Free, no limits. Requires an App Password.",
      ...
      "editor.email.appPasswordHint": "16-character password from Google. NOT your regular password.",
      "editor.email.sendgridTitle": "SendGrid",
      "editor.email.sendgridDesc": "Professional email service. Free tier: 100 emails/day. Great deliverability.",
      ...
      "editor.email.resendDesc": "Modern email API for developers. Free tier: 100 emails/day. Configured via .env.",
      ...
      "editor.db.modelHint": "Prisma model name in lowercase. Must match your schema.",
      "editor.db.argsHint": "Prisma query arguments as JSON. Format depends on operation.",
      "editor.db.exampleFor": "Example for",
      "editor.db.useTemplate": "← Use this template",
      "editor.db.autoDataHint": "💡 For 'create': if args is empty, data from the previous step is used as 'data' field.",
      ...
      "editor.transform.expressionHint": "JavaScript expression. 'input' contains data from previous step.",
      "editor.transform.examples": "Examples (click to use)",
      "editor.transform.mappingPlaceholder": '{\n  "userName": "input.user.name",\n  "userEmail": "input.user.email"\n}',
      "editor.transform.mappingHint": "JSON object mapping output fields to input paths.",
      "editor.transform.mappingBuilder": "Quick builder",
      "editor.transform.addField": "Add field",
      "editor.transform.dataFlowHint": "💡 Result of transformation becomes input for the next step. Use 'input' to access previous step data.",
      ...
      "editor.webhook.description": "External services send HTTP POST requests to your unique URL. The request body becomes input data for subsequent workflow steps.",
      "editor.webhook.urlHint": "Send POST requests to this URL with JSON body to trigger the workflow.",
      "editor.webhook.copy": "Copy",
      "editor.webhook.generate": "Generate URL",
      "editor.webhook.saveFirst": "Save workflow to generate your unique webhook URL",
      "editor.webhook.usageExamples": "Usage examples",
      "editor.webhook.dataFlowHint": "💡 JSON body from POST request becomes 'input' for the next step. Example: if you send {\"userId\": 123}, use input.userId in the next block.",

      // Schedule Trigger
      "editor.schedule.howItWorks": "How it works",
      "editor.schedule.description": "Workflow runs automatically based on cron schedule. Perfect for periodic tasks like reports, cleanups, or sync jobs.",
      "editor.schedule.cronHint": "Cron expression defines when workflow runs. Click presets below for common schedules.",
      ...
      "editor.schedule.timezoneHint": "All times are calculated in this timezone. Click presets below.",
      "editor.schedule.dataFlowHint": "💡 Scheduled runs pass empty {} as input. Use Transformation or HTTP Request to fetch initial data.",

      // Email Trigger (IMAP Polling)
      "editor.emailTrigger.howItWorks": "How it works",
      "editor.emailTrigger.imapDescription": "Connect your email account directly. We'll check for new emails every 5 minutes and trigger workflow automatically.",
      "editor.emailTrigger.imapEmail": "Email Address",
      "editor.emailTrigger.imapEmailHint": "Your email address (Gmail, Mail.ru, Yandex, Yahoo, Outlook, etc.)",
      "editor.emailTrigger.imapPassword": "App Password",
      "editor.emailTrigger.appPasswordPlaceholder": "Your app password (not regular password)",
      "editor.emailTrigger.appPasswordHint": "Use an App Password, NOT your regular password. See instructions below.",
      ...
      "editor.emailTrigger.mailruInstructions": "Settings → Password & Security → App Passwords",
      "editor.emailTrigger.yandexInstructions": "Settings → Security → App Passwords",
      ...
      "editor.emailTrigger.imapServerHint": "Leave empty for auto-detection (Gmail, Mail.ru, Yandex, etc.)",
      ...
      "editor.emailTrigger.filterSubjectPlaceholder": "e.g., Order confirmation",
      ...
      "editor.emailTrigger.pollingInfo": "📬 Emails are checked every 5 minutes. New emails trigger workflow with email data (from, subject, body) as input.",
    ...
    ru: {
      ...
      // HTTP Request action
      "editor.http.urlHint": "Полный URL включая https://. Можно добавить параметры запроса.",
      "editor.http.methodHint": "GET — получить, POST — отправить, PUT — обновить, DELETE — удалить",
      "editor.http.headersPlaceholder": '{"Authorization": "Bearer token", "X-Custom": "значение"}',
      "editor.http.headersHint": "JSON объект с заголовками. Content-Type добавляется автоматически.",
      "editor.http.bodyPlaceholder": '{"key": "value"} или оставьте пустым для передачи данных предыдущего шага',
      "editor.http.bodyHint": "Оставьте пустым, чтобы автоматически отправить данные предыдущего шага.",
      ...
      // Email action
      "editor.email.toHint": "Email получателя. Можно использовать данные из предыдущих шагов.",
      ...
      "editor.email.htmlPlaceholder": "<h1>Привет!</h1>\n<p>Ваш workflow успешно выполнен.</p>",
      "editor.email.htmlHint": "HTML контент с форматированием. Используйте теги <b>, <p>, <a>.",
      "editor.email.textPlaceholder": "Привет!\n\nВаш workflow успешно выполнен.",
      "editor.email.textHint": "Текстовая версия для почтовых клиентов без поддержки HTML.",
      "editor.email.autoContentHint": "💡 Если контент пустой, данные предыдущего шага отправятся как JSON автоматически.",
      "editor.email.provider": "Провайдер email",
      "editor.email.gmailTitle": "Gmail SMTP",
      "editor.email.gmailDesc": "Отправляйте письма напрямую с вашего Gmail через SMTP. Бесплатно, без лимитов. Требует App Password.",
      ...
      "editor.email.appPassword": "App Password",
      "editor.email.appPasswordHint": "16-символьный пароль от Google. НЕ ваш обычный пароль.",
      "editor.email.sendgridTitle": "SendGrid",
      "editor.email.sendgridDesc": "Профессиональный email сервис. Бесплатно: 100 писем/день. Отличная доставляемость.",
      ...
      "editor.email.resendTitle": "Resend",
      "editor.email.resendDesc": "Современный email API для разработчиков. Бесплатно: 100 писем/день. Настраивается через .env.",
      ...
      "editor.db.modelHint": "Имя модели Prisma в нижнем регистре. Должно совпадать со схемой.",
      "editor.db.argsHint": "Аргументы запроса Prisma в формате JSON. Формат зависит от операции.",
      "editor.db.exampleFor": "Пример для",
      "editor.db.useTemplate": "← Использовать шаблон",
      "editor.db.autoDataHint": "💡 Для 'create': если args пустой, данные предыдущего шага используются как 'data'.",
      ...
      "editor.transform.expressionPlaceholder": "input.user.name.toUpperCase()",
      "editor.transform.expressionHint": "JavaScript выражение. 'input' содержит данные предыдущего шага.",
      "editor.transform.examples": "Примеры (нажмите для использования)",
      "editor.transform.mappingPlaceholder": '{\n  "userName": "input.user.name",\n  "userEmail": "input.user.email"\n}',
      "editor.transform.mappingHint": "JSON объект, сопоставляющий поля вывода с путями ввода.",
      "editor.transform.mappingBuilder": "Быстрый конструктор",
      "editor.transform.addField": "Добавить поле",
      "editor.transform.dataFlowHint": "💡 Результат трансформации становится входом следующего шага. Используйте 'input' для доступа к данным.",
      ...
      "editor.webhook.description": "Внешние сервисы отправляют HTTP POST запросы на ваш уникальный URL. Тело запроса становится входными данными для последующих шагов workflow.",
      "editor.webhook.urlHint": "Отправляйте POST запросы на этот URL с JSON телом для запуска workflow.",
      "editor.webhook.copy": "Копировать",
      "editor.webhook.generate": "Сгенерировать URL",
      "editor.webhook.saveFirst": "Сохраните workflow для генерации уникального webhook URL",
      "editor.webhook.usageExamples": "Примеры использования",
      "editor.webhook.dataFlowHint": "💡 JSON тело POST запроса становится 'input' для следующего шага. Например: если отправить {\"userId\": 123}, используйте input.userId в следующем блоке.",

      // Schedule Trigger
      "editor.schedule.howItWorks": "Как это работает",
      "editor.schedule.description": "Workflow запускается автоматически по cron расписанию. Идеально для периодических задач: отчёты, очистка, синхронизация.",
      "editor.schedule.cronHint": "Cron выражение определяет когда запускается workflow. Нажмите на пресеты ниже для частых вариантов.",
      ...
      "editor.schedule.timezoneHint": "Всё время рассчитывается в этом часовом поясе. Нажмите на пресеты ниже.",
      "editor.schedule.dataFlowHint": "💡 Запуски по расписанию передают пустой {} как input. Используйте Transformation или HTTP Request для получения начальных данных.",

      // Email Trigger (IMAP Polling)
      "editor.emailTrigger.howItWorks": "Как это работает",
      "editor.emailTrigger.imapDescription": "Подключите свою почту напрямую. Мы проверяем новые письма каждые 5 минут и автоматически запускаем workflow.",
      "editor.emailTrigger.imapEmail": "Email адрес",
      "editor.emailTrigger.imapEmailHint": "Ваш email (Gmail, Mail.ru, Yandex, Yahoo, Outlook и др.)",
      "editor.emailTrigger.imapPassword": "Пароль приложения",
      "editor.emailTrigger.appPasswordPlaceholder": "Ваш пароль приложения (не обычный пароль)",
      "editor.emailTrigger.appPasswordHint": "Используйте Пароль приложения, НЕ обычный пароль. См. инструкции ниже.",
      "editor.emailTrigger.appPasswordTitle": "⚠️ Как создать Пароль приложения",
      "editor.emailTrigger.createAppPassword": "Создать пароль приложения",
      "editor.emailTrigger.mailruInstructions": "Настройки → Пароли и безопасность → Пароли приложений",
      "editor.emailTrigger.yandexInstructions": "Настройки → Безопасность → Пароли приложений",
      ...
      "editor.emailTrigger.imapServerHint": "Оставьте пустым для автоопределения (Gmail, Mail.ru, Yandex и др.)",
      ...
      "editor.emailTrigger.filterSubjectPlaceholder": "напр., Подтверждение заказа",
      ...
      "editor.emailTrigger.pollingInfo": "📬 Почта проверяется каждые 5 минут. Новые письма запускают workflow с данными письма (от, тема, текст) как input.",
